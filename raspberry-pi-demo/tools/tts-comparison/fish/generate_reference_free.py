...
from comparison_utils import merge_generation_records, validate_generation_request
...
DEFAULT_SEEDS = tuple(range(4201, 4221))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate reproducibly seeded reference-free Fish Speech WAVs."
    )
    parser.add_argument(
        "--checkpoint-path",
        type=Path,
        default=ROOT / "models" / "s2-pro",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def main() -> None:
    ...
    for seed in seeds:
        ...
        output_path = args.output_dir / f"fish_reference_free_seed_{seed}.wav"
        ...
        record = {
            ...
            "text": TEXT,
            "referenceAudio": None,
            "nativePath": str(output_path.relative_to(ROOT)).replace("\\", "/"),
        ...
        del audio, audio_array, merged_codes, code_chunks
    ...
    summary = {
        ...
        "modelPath": str(args.checkpoint_path),
        "text": TEXT,
