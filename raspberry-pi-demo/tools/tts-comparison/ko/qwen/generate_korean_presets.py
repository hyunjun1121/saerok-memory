"""Generate Haru's Korean prompt with every Qwen CustomVoice preset.
...
EXISTING_TTS_DIR = HERE.parents[2] / "tts"
SNAPSHOT = (
    EXISTING_TTS_DIR
    / ".cache"
    / "huggingface"
...
def main() -> int:
    ...
    if not expected.issubset(actual):
        raise RuntimeError(f"Required Korean speaker missing: actual={runtime_speakers}")
    ...
    for index, (speaker, slug) in enumerate(VOICES, start=1):
        ...
        verification_path = VERIFY_DIR / f"{index:02d}-{slug}.json"
        if wav_path.is_file() and ogg_path.is_file() and verification_path.is_file():
            cached = json.loads(verification_path.read_text(encoding="utf-8"))
            if (
                cached.get("speaker") == speaker
                and cached.get("text") == TEXT
                and cached.get("language") == LANGUAGE
                and cached.get("seed") == SEED
                and cached.get("wav", {}).get("sha256") == sha256_file(wav_path)
                and cached.get("ogg", {}).get("sha256") == sha256_file(ogg_path)
            ):
                ...
                print(f"[{index}/{len(VOICES)}] Reusing verified {speaker}", flush=True)
                continue
    ...
    method = {
        "id": "qwen",
        "name": "Qwen3-TTS 한국어 preset 음색 비교",
        "model": {
            "id": MODEL_ID,
            "revision": MODEL_REVISION,
            "license": LICENSE,
        ...
        "voiceInventory": {
            ...
            "total": len(VOICES),
            "selectionRationale": (
                "The official model supports Korean with all 9 preset speakers, so all 9 "
                "were generated using the same Korean prompt. Sohee is the Korean-native preset."
            ),
        },
        ...
        "samples": [
            {
                ...
                "label": f"{sample['speaker']} · {NATIVE_LANGUAGE_LABELS[sample['speaker']]} 원어 음색",
                ...
                "note": (
                    "한국어 원어 preset"
                    if sample["speaker"] == "Sohee"
                    else (
                        f"원어 음색은 {NATIVE_LANGUAGE_LABELS[sample['speaker']]}이며, "
                        "모델이 공식 지원하는 한국어로 동일 문장을 생성함."
                    )
                ),
                **(
                    {
