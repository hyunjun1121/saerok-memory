...
def validate_runtime_provenance(
    *,
    source_head: str,
    source_dirty: bool,
    model_metadata_revisions: Sequence[str],
    ...
    if source_head != expected_source_revision:
        raise ValueError(
            f"source HEAD mismatch: expected {expected_source_revision}, found {source_head}"
        )
    if source_dirty:
        raise ValueError("source checkout is dirty")
    if (
        len(model_metadata_revisions) != expected_model_file_count
        or set(model_metadata_revisions) != {expected_model_revision}
    ):
        raise ValueError(
            "model snapshot revision mismatch: "
            f"expected {expected_model_file_count} files at {expected_model_revision}"
        )
...
def validate_generation_provenance(
    ...
    *,
    ...
    hash_seed_set = set(int(seed) for seed in native_sha256)
    if native_seed_set != expected_set or len(native_paths) != len(expected):
        raise ValueError(
            f"Native seed set mismatch: expected {list(expected)}, "
            f"found {sorted(native_seed_set)}"
        )
    if hash_seed_set != expected_set or len(native_sha256) != len(expected):
        raise ValueError("Native SHA-256 seed set mismatch")
    ...
    for record in normalized_records:
        ...
        if record.get("nativePath") != native_paths[seed]:
            raise ValueError(f"Seed {seed} nativePath does not match native WAV")
        if record.get("nativeSha256") != native_sha256[seed]:
            raise ValueError(f"Seed {seed} nativeSha256 does not match native WAV")
