...
def process_candidate(native_path: Path) -> dict[str, object]:
    ...
    final_path = CANDIDATE_OGG_DIR / f"fish_ko_reference_free_seed_{seed}.ogg"
...
def archive_existing_generated() -> None:
    groups = {
        "audio": [path for path in AUDIO_DIR.iterdir() if path.is_file()],
        "rejected": [path for path in REJECTED_DIR.iterdir() if path.is_file()],
        "candidates-wav": [
            path for path in CANDIDATE_WAV_DIR.iterdir() if path.is_file()
        ],
        "candidates-ogg": [
            path for path in CANDIDATE_OGG_DIR.iterdir() if path.is_file()
        ],
    }
    metadata_path = ROOT / "outputs" / "postprocess-metadata.json"
    if metadata_path.exists():
        groups["metadata"] = [metadata_path]
...
def main() -> None:
    ...
    native_paths = sorted(NATIVE_DIR.glob("fish_ko_reference_free_seed_*.wav"))
    ...
    native_hash_by_seed = {
        int(path.stem.rsplit("_", maxsplit=1)[-1]): file_sha256(path)
        for path in native_paths
    }
    ...
    validate_generation_provenance(
        ...
        native_sha256=native_hash_by_seed,
