...
export const DEFAULT_PORT = 4191;
...
function isPublicComparisonPath(path) {
  ...
  if (path === 'qwen/REPORT.md' || path === 'fish/REPORT.md') return true;
  return /^(?:qwen|fish)\/audio\/[A-Za-z0-9._-]+\.(?:ogg|wav)$/u.test(path);
}
...
export function createComparisonServer(rootDirectory = defaultRoot) {
  ...
  return createServer((request, response) => {
    ...
    try {
      ...
      sendText(response, error instanceof URIError ? 400 : 400, error instanceof URIError ? 'Malformed URL' : 'Invalid URL');
      return;
    }
...
if (isCli) {
  ...
  server.listen(port, '127.0.0.1', () => {
    console.log(`Haru Korean TTS comparison: http://127.0.0.1:${port}`);
