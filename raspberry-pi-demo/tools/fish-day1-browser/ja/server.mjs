...
function isPublicPath(path) {
  return new Set(["index.html", "styles.css", "app.js", "day1-inventory.json", "start-greeting-inventory.json"]).has(path)
    || /^audio\/[A-Za-z0-9._-]+\.mp3$/u.test(path);
}
...
export function createJapaneseDay1SelectorServer({
  ...
  const inventory = JSON.parse(readFileSync(join(root, "day1-inventory.json"), "utf8"));
  ...
  return createServer(async (request, response) => {
    ...
    if (pathname === "/api/selections") {
      if (request.method === "GET") {
        try { sendJson(response, 200, readSelectionDocument(selectionsPath, inventory)); }
      ...
      if (request.method === "POST") {
        try {
          ...
          const document = buildSelectionDocument(inventory, payload?.selections);
    ...
    if (!isPublicPath(relativePath)) { sendText(response, 404, "Not found"); return; }
    const lexicalPath = resolve(join(root, relativePath));
    if (!isContained(root, lexicalPath) || !existsSync(lexicalPath)) { sendText(response, 404, "Not found"); return; }
    let filePath;
    let fileStat;
    try { filePath = realpathSync.native(lexicalPath); fileStat = statSync(filePath); }
    catch { sendText(response, 404, "Not found"); return; }
    if (!isContained(root, filePath) || !fileStat.isFile()) { sendText(response, 404, "Not found"); return; }
    ...
    stream.on("error", (error) => { if (response.headersSent) response.destroy(error); else sendText(response, 500, "Read failed"); });
...
if (isCli) {
  const port = Number.parseInt(process.argv[2] ?? "4194", 10);
  if (!Number.isInteger(port) || port < 1024 || port > 65535) throw new Error("Port must be an integer between 1024 and 65535");
  const server = createJapaneseDay1SelectorServer();
  server.listen(port, "127.0.0.1", () => {
    console.log(`Haru Day 1 Japanese TTS selector: http://127.0.0.1:${port}`);
