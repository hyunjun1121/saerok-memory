...
export function createDay1SelectorServer({
  ...
  const inventoryPath = join(root, "day1-inventory.json");
  ...
  return createServer(async (request, response) => {
    ...
    if (pathname === "/api/selections") {
      if (request.method === "GET") {
        try {
          sendJson(response, 200, readSelectionDocument(selectionsPath, inventory));
        } catch (error) {
...
if (isCli) {
  const port = Number.parseInt(process.argv[2] ?? "4192", 10);
  if (!Number.isInteger(port) || port < 1024 || port > 65535) {
    throw new Error("Port must be an integer between 1024 and 65535");
  }
  const server = createDay1SelectorServer();
  server.listen(port, "127.0.0.1", () => {
    console.log(`Haru Day 1 TTS selector: http://127.0.0.1:${port}`);
