...
앱을 열면 기존 활동 시작 화면보다 먼저 `로그인 대기` 화면(일본어 build는 `ログイン待ち`)이 표시됩니다. 화면 진입 시 카드 대기 안내 음성이 자동 재생되고, USB NFC 리더기에 이용자 카드를 대면 리더기가 키보드처럼 숫자 `5`를 한 번 보내 활동 시작 화면으로 넘어갑니다. 안내를 다시 듣고 싶으면 카드 인증 전 물리 버튼 `1`~`4` 중 아무 버튼이나 누릅니다. 이 버튼은 음성만 다시 재생하며, 카드 인증 전에는 활동을 시작하지 않습니다.
