...
이 문서는 `deep-research-report.md`와 현재 코드 구조를 대조해 Haru에 어떤 인지 과제를 우선 추가할지 정리한 실행 판단서이다. 목표는 정식 임상 검사를 복제하는 것이 아니라, 고령 사용자가 매일 부담 없이 수행할 수 있는 근거 기반의 인지 루틴을 늘리고 보호자·상담사가 참고할 활동 데이터를 축적하는 것이다.

Haru는 의료 진단, 공식 질병 위험 판정, MMSE/MoCA/ACE-III 점수 산출, 공식 검사 자극물 복제를 하지 않는다. 다만 기존처럼 모든 판단을 피하는 방어적 방향은 버리고, 신뢰 가능한 의료·인지과학 자료를 바탕으로 Haru 자체의 주의도·참고 위험도 평가를 구축한다. 결과 표현은 “연습 기록”, “참여 흐름”, “대화 준비 자료”, “Haru 자체 주의도”, “필요 시 전문가 상담 권유”로 확장한다.

프로그램 실행 또는 첫 사용 시에는 Haru의 자체 평가는 의학적 진단, 공식 선별검사, 치료나 예방 효과가 아니라는 안내를 표시한다. 그 전제 위에서 반복 루틴 데이터, 회상 변화, 반응시간, 오류 패턴, 그리기 telemetry, 의미 유창성, 보호자 관찰을 조합해 사용자가 합리적으로 참고할 수 있는 신호를 제공한다.
...
또한 기존 `delayed_word_recall` mock lesson을 3단어에서 5단어 지연회상으로 바꿨다. 이는 공식 검사 자극물 복제가 아니라 Haru용 단순 단어 세트이며, 화면에는 의료 점수나 cutoff를 표시하지 않는다. 이번 반영에서는 각 단어에 Haru 원문 범주 단서를 붙이고, encoding/recall 단계에서 범주 단서, 계획 지연 시간, 실제 관찰 지연 시간, 선택 답안, 기대 답안, 맞게 선택한 개수를 `cognitiveRoutineResults` metadata로 저장하도록 확장했다. 이어서 recall 단계에는 사용자가 먼저 떠올린 단어를 적는 자유회상 입력을 추가했고, 이후 기존 선택형 재인 확인을 계속하도록 했다.

이번 이어진 반영에서는 `verbal_fluency_practice`를 추가했다. 첫 버전은 “동물” 범주에서 떠오르는 단어를 적는 방식이며, 30초 타이머와 단어 추가 UI, 서로 다른 단어 수와 반복 단어 수를 기록한다. 이 값은 언어 유창성 루틴의 활동 metadata일 뿐이며, 진단 점수나 임상 cutoff로 표시하지 않는다.

보호자 변화체크는 `/family`의 보호자 탭에 “보호자 관찰 메모”로 추가했다. 공식 AD8 문항·채점표를 복제하지 않고, Haru 원문 관찰 영역과 자유 메모를 저장한다. 이번 확장에서는 각 영역에 대해 “큰 변화 없음 / 가끔 달라 보임 / 자주 달라 보임 / 잘 모르겠음” 수준의 구조화 응답도 함께 저장하도록 바꿨다. 저장된 최신 관찰은 상담사 탭에도 표시되어 다음 상담 전 대화 준비 자료로 활용된다.

디지털 실행기능 루틴으로 `trail_switching_practice`를 추가했다. 정식 Trail Making Test A/B 양식이나 채점 규준을 복제하지 않고, 숫자와 그림 단서를 번갈아 누르는 TMT-lite 형태로 구성했다. 저장값은 완료 여부, 클릭 순서, 오류 수, 소요시간이며, 상담사 화면에서는 질병 위험이 아니라 주의 전환 활동 기록으로만 해석해야 한다.

이번 반영에서는 기존 `shape_copy_practice`를 dCDT-lite 방향으로 확장했다. 공식 시계그리기 문항이나 채점표를 복제하지 않고, 단순 도형 따라 그리기 안에서 획 수, 첫 터치까지 걸린 시간, 그리기 지속 시간, 멈춤 횟수, 지우기 횟수, 경로 길이, 샘플링된 터치 경로를 저장한다. 이 값은 추후 보호자·상담사 리포트의 시공간/운동 참여 흐름을 구성하기 위한 원자료이며, 화면에서는 진단 점수나 정상/이상 판정을 표시하지 않는다.

날짜·요일 지남력의 기본층으로 `orientation_practice`를 추가했다. 정식 MMSE, MoCA, GPCOG 문항이나 점수표를 복제하지 않고, 오늘 날짜와 요일을 선택형으로 확인한다. 저장값은 기대 날짜, 선택 날짜, 일치 여부, 응답 시간, locale이며, 화면과 리포트에서는 진단 점수 대신 “날짜 감각 루틴 참여 기록”으로만 표현한다. 장소 지남력은 위치권한과 개인정보 이슈가 있어 MVP에서는 보류한다.

저장 metadata는 임상 점수가 아니라 다음 개발을 위한 루틴 기록이다.
...
3. **Haru 자체 주의도/참고 위험도 모델 정교화**:
   - 단일 데이터가 아닌 다중 약한 신호(단어 지연회상률 변화, 반응 속도 편차, 그리기 궤적 불규칙성, 보호자 관찰 메모의 영역별 빈도)를 복합 평가하는 가중치 룰엔진을 정교화합니다.
   - 가중치를 조정할 수 있는 설정 옵션을 개발하여 상담 전문가가 맞춤형으로 참고 위험도를 조율할 수 있도록 기여합니다.
